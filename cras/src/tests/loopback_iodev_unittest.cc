  ...
  protected:
    virtual void SetUp() {
      ...
      shm_->area = static_cast<struct cras_audio_shm_area *>(
          calloc(1, kBufferFrames * kFrameBytes * 2 + sizeof(*shm_->area)));
      memset(shm_->area->samples,
             0x55,
             kBufferFrames * kFrameBytes);
      cras_shm_set_frame_bytes(shm_, kFrameBytes);
      cras_shm_set_used_size(shm_, kBufferFrames * kFrameBytes);

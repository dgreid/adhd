...
fn main() -> std::result::Result<(), Box<std::error::Error>> {
    ...
    let mut frames_in: Vec<[f32; 2]> = Vec::with_capacity(num_frames as usize);
    let mut buf = [0u8; 4];
    ...
    let out_signal = signal::from_iter(frames_in).dc_block(0.995);

    let mut frames_out: Vec<[f32; 2]> = Vec::with_capacity(num_frames as usize);
    let start_time = Instant::now();
    for (f, v) in frames_out.iter_mut().zip(out_signal.until_exhausted()) {
        *f = v;
    }
    ...
    for f in signal::from_iter(frames_out)
        ...
        .until_exhausted()
    {
        out_file.write(&f[0].to_le_bytes())?;
        out_file.write(&f[1].to_le_bytes())?;
    }
    println!(
